...
layout = html.Div([html.Div(id="orders-content")])
...
def update_orders(
    ...
    df_placed = fetch_daytime_data(
        selected_store_name, start_date, end_date, account_filter, day_val
    )
    df_collected = fetch_collection_data(
        selected_store_name, start_date, end_date, account_filter, day_val
    )
    df_totals = fetch_order_totals(
        selected_store_name, start_date, end_date, account_filter
    )

    if df_placed.empty and df_collected.empty:
        return html.Div(
            "No daytime or collection data available.",
            style={"color": "#7FDBFF", "textAlign": "center", "marginTop": "40px"},
        )
    ...
    fig_collected = px.bar(
        ...
        labels={
            "collected_hour": "Hour of Day (24h)",
            "customer_category": "Category",
            "order_count": "Orders",
        },
        template="plotly_dark",
    ...
    if not df_totals.empty:
        ...
        median_total = df_hist_data["Total"].median()
        ...
        if not pd.isna(median_total):
            fig_hist.add_vline(
                x=median_total,
                line_width=2,
                line_dash="dash",
                line_color="white",
                annotation_text=f"Median: ${median_total:.2f}",
        ...
        fig_hist = px.scatter(
            title="No order total data available.", template="plotly_dark"
        )
        fig_hist.update_layout(
    ...
    return html.Div(
        [
            html.Div(
                [
                    html.Div(
                        [dcc.Graph(id="orders-placed-histogram", figure=fig_placed)],
                        style={"width": "50%", "display": "inline-block"},
                    ),
                    html.Div(
                        [
                            dcc.Graph(
                                id="orders-collected-histogram", figure=fig_collected
                            )
                        ],
                        style={"width": "50%", "display": "inline-block"},
                    ),
