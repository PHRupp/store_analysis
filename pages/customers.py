...
from database_utils import fetch_customer_stats, fetch_top_customers, fetch_overdue_customers, fetch_new_customers_trend, fetch_last_order_trend
...
def update_customers(selected_store, account_filter, start_date, end_date):
    ...
    fig_count = px.pie(
        df_cust, values='customer_count', names='customer_category', color='customer_category',
        title='Customer Distribution by Category', template='plotly_dark', hole=0.4,
        category_orders={'customer_category': CATEGORY_ORDER}, color_discrete_map=CATEGORY_COLORS
    )

    fig_spend = px.pie(
        df_cust, values='total_spend', names='customer_category', color='customer_category',
        title='Total Spend by Category', template='plotly_dark', hole=0.4,
        category_orders={'customer_category': CATEGORY_ORDER}, color_discrete_map=CATEGORY_COLORS
    )
    ...
    fig_top = px.bar(
        df_top, x='Name', y='total_spend', title='Top Customers: Lifetime Value vs Median Order Amount',
        labels={'total_spend': 'Total Spending'}, custom_data=custom_hover_cols, template='plotly_dark'
    )
    fig_top.update_traces(marker_color='#00CC96', hovertemplate="Total Spend: $%{y:,.2f}<br>" + common_hover, selector=dict(type='bar'))
    fig_top.add_trace(go.Scatter(
        x=df_top['Name'], y=df_top['median_spend'], name='Median Spend', line=dict(color='#7FDBFF', width=3),
        mode='lines+markers', customdata=df_top[custom_hover_cols], hovertemplate="Median Spend: $%{y:,.2f}<br>" + common_hover, yaxis='y2'
    ))
    fig_top.update_layout(hovermode='x unified', yaxis=dict(title='Total Spending ($)'), yaxis2=dict(title='Median Spend ($)', overlaying='y', side='right', showgrid=False, color='#7FDBFF'))

    overdue_hover_cols = ['median_spend', 'order_count', 'median_days_between_orders', 'recency', 'total_spend']
    fig_overdue = px.bar(
        df_overdue, x='Name', y='days_past_expected', color='customer_category',
        category_orders={'customer_category': CATEGORY_ORDER}, color_discrete_map=CATEGORY_COLORS,
        title='Customers Past Expected Visit (Days)', labels={'days_past_expected': 'Days Overdue', 'Name': 'Customer', 'customer_category': 'Category'},
        custom_data=overdue_hover_cols, template='plotly_dark'
    )
    fig_overdue.update_traces(hovertemplate="<b>%{x}</b><br>Days Overdue: %{y:.0f}<br>Total Lifetime Spend: $%{customdata[4]:,.2f}<br>Median Order Total: $%{customdata[0]:,.2f}<br>Order Count: %{customdata[1]}<br>Median Interval: %{customdata[2]:.1f} days<br>Last Order: %{customdata[3]:.0f} days ago<extra></extra>")
    ...
    if not df_returning.empty:
        fig_returning = px.bar(
            df_returning, x='month_year', y='last_order_count', color='customer_category', title='Last Order Activity Trend',
            labels={'month_year': 'Month', 'last_order_count': 'Customers', 'customer_category': 'Category'},
            category_orders={'customer_category': CATEGORY_ORDER}, color_discrete_map=CATEGORY_COLORS, template='plotly_dark'
        )
        fig_returning.update_layout(plot_bgcolor='#111111', paper_bgcolor='#111111', font_color='#7FDBFF', legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    else:
    ...
    return html.Div([
        html.Div([
            html.Div([dcc.Graph(id='cust-count-pie', figure=fig_count)], style={'width': '50%', 'display': 'inline-block'}),
            html.Div([dcc.Graph(id='cust-spend-pie', figure=fig_spend)], style={'width': '50%', 'display': 'inline-block'})
        ], style={'display': 'flex'}),
        html.Div([
            html.Div([dcc.Graph(id='cust-top-bar-line', figure=fig_top)], style={'width': '50%'}),
            html.Div([dcc.Graph(id='cust-overdue-bar', figure=fig_overdue)], style={'width': '50%'})
        ], style={'display': 'flex'}),
        html.Div([
            html.Div([dcc.Graph(id='cust-new-trend', figure=fig_new)], style={'width': '50%'}),
            html.Div([dcc.Graph(id='cust-last-order-trend', figure=fig_returning)], style={'width': '50%'})
        ], style={'display': 'flex'}),
    ])